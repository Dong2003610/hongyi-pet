...
test('settings accept only complete, finite, whitelisted values', () => {
  const valid = { edgeSnap: true, alwaysOnTop: true, typingReaction: false, clickThrough: false, petScale: 0.8, openAtLogin: false, soundEnabled: true, sedentaryReminder: true };
...
test('persisted stats and reminders reject malformed values', () => {
  ...
  assert.throws(() => parseReminders([{ id: 'x', text: '', dueAt: new Date().toISOString(), createdAt: new Date().toISOString(), repeat: 'none' }]), /text/);
});
