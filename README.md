...
- 🤖 AI 聊天（可选，接入 OpenAI 兼容接口；未配置时自动回退本地规则回复）
  - 💬 右键和面板互动也支持 AI 短句：结合摸头、投喂、散步、睡觉等动作回应；聊天轮换今日一句、俏皮话、小问题和小挑战
  - 📅 实时日期时间注入，回答准确
...
右键互动复用同一份 AI 配置。动作和本地短句立即出现，AI 回复随后更新气泡；断网、未配置或请求超时仍可正常互动。本地回复会避开最近 8 句，AI 回复最多 40 字并自动换行。快速连点时会限制请求频率，新动作会取消旧回复。每日一句是 AI 原创陪伴短句，不冒充名人名言。

互动 AI 只接收宠物名字、性格、心情、好感度、时间、当前动作和最近互动短句，不接入普通聊天记录或待办列表。

## 验证互动

在 `app` 目录运行 `npm test` 检查动作和 AI 请求逻辑；运行 `npm run test:pet-ui` 会编译当前桌宠界面，在独立的隐藏 Electron 窗口验证跳跃、落地、气泡及连续互动。界面测试使用模拟回复，不调用真实 AI，截图输出到 `app/qa/`。
